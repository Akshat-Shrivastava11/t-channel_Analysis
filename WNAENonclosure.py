...
def GetSVJbins(DNN_inner_edge,MET_inner_edge):
    ...
    SVJbins = {
                ...
                "3SVJ": [DNN_inner_edge,MET_inner_edge],
    }
    return SVJbins
...
def getData(path, scale=1.0, year = "2018"):
    ...
    Data = [
        info.DataSetInfo(basedir=path, fileName="plots_WNAE_data.root",        sys= -1.0, label="Data",        scale=scale),
    ]
    bgData = [
        ...
        info.DataSetInfo(basedir=path, fileName="plots_WNAE.root",               label="all bckg",                        scale=scale, color=ROOT.TColor.GetColor("#9c9ca1")),
    ]
    sgData = [
...
def GetABCDhistPerSVJBin(data, ABCDhistoVar, Year, SVJbin,DNN_inner_edge,DNN_outer_edge,MET_inner_edge,MET_outer_edge):
    ...
    SVJ, (dnn, met) = SVJbin
    histName = ABCDhistoVar + Year + SVJ+"_WNAE"

    print(f'HIST NAME = {histName}')
...
def get_ABCD_counts_by_SVJ(Data,sgData, bgData, ABCDHistoVar, Year,SVJBins,DNN_inner_edge,DNN_outer_edge,MET_inner_edge,MET_outer_edge):
    """
    Extracts the ABCD region counts split by the number of SVJ jets for both signal and background.
    """
    counts_by_SVJ = {}
    error_by_SVJ = {}
    for data_list, label in [(Data,"Data"), (bgData, "Background")]:
        for data in data_list:  # Iterate over individual dataset objects
            for svj_bin in SVJBins.items():
                #hist_dict = GetABCDhistPerSVJBin(data, ABCDHistoVar, Year, svj_bin)
                hist_dict = GetABCDhistPerSVJBin(data, ABCDHistoVar, Year, svj_bin,DNN_inner_edge,DNN_outer_edge,MET_inner_edge,MET_outer_edge)
                #print('hist_dict:',hist_dict)
                #put if statment with pre and if Year is pre Data = Data SR SR cut s now pre
...
def compute_ABCD_prediction(Data, sgData, bgData, ABCDHistoVar, Year, DNN_inner_edges, DNN_outer_edges, MET_inner_edges, MET_outer_edges,VR):
    outer_edge_results = {}  
    ...
    for DNN_inner_edge,DNN_outer_edge,MET_inner_edge,MET_outer_edge in zip(DNN_inner_edges,DNN_outer_edges,MET_inner_edges,MET_outer_edges):
        ...
        counts_by_SVJ, error_by_SVJ = get_ABCD_counts_by_SVJ(Data, sgData, bgData, ABCDHistoVar, Year, SVJBins, DNN_inner_edge, DNN_outer_edge, MET_inner_edge, MET_outer_edge)
        ...
        for ((svj_bin, label), (N_A, N_B, N_C, N_D)), ((_, _), (NA_err, NB_err, NC_err, ND_err)) in zip(counts_by_SVJ.items(), error_by_SVJ.items()):
            ...
            if "Data" in label:
                #print('appending stuff for Data')
                obs_data.append(N_A)
                ...
                #print('appending stuff for Bckg')
                obs_bg.append(N_A)
...
def plot_ABCD_ratios(
    ...
    #print(f'ratio 0svj {ratio_0SVJ_data}')
    ...
    def plot_and_save(x_values,non_closures,nonclosure_err, diff_ratios,diff_error, labels, colors,title,filename):
        ...
        ax1.set_title(f'Difference in Data and Background Ratios', fontsize=18)
...
def main():
    ...
    Years = ['2016_','2017_','2018_']
    for Year in Years:
        VR = 'VR1'
        output_dir = f'Nonclosure/WNAE/VRI/{Year}/ControlRegion'
        ...
        outer_edge_results,obs_0SVJ_data, pred_0SVJ_data, obs_0SVJ_sig, pred_0SVJ_sig, obs_0SVJ_bg, pred_0SVJ_bg,obs_1SVJ_data, pred_1SVJ_data, obs_1SVJ_sig, pred_1SVJ_sig, obs_1SVJ_bg, pred_1SVJ_bg,obs_2PSVJ_data, pred_2PSVJ_data, obs_2PSVJ_sig, pred_2PSVJ_sig, obs_2PSVJ_bg, pred_2PSVJ_bg,obs_3PSVJ_data, pred_3PSVJ_data, obs_3PSVJ_sig, pred_3PSVJ_sig, obs_3PSVJ_bg, pred_3PSVJ_bg,obserr_0SVJ_data, obserr_0SVJ_sig, obserr_0SVJ_bg,prederr_0SVJ_data, prederr_0SVJ_sig, prederr_0SVJ_bg,obserr_1SVJ_data, obserr_1SVJ_sig, obserr_1SVJ_bg,prederr_1SVJ_data, prederr_1SVJ_sig, prederr_1SVJ_bg,obserr_2PSVJ_data, obserr_2PSVJ_sig, obserr_2PSVJ_bg,prederr_2PSVJ_data, prederr_2PSVJ_sig, prederr_2PSVJ_bg,obserr_3PSVJ_data, obserr_3PSVJ_sig, obserr_3PSVJ_bg, prederr_3PSVJ_data, prederr_3PSVJ_sig, prederr_3PSVJ_bg, = compute_ABCD_prediction(Data, sgData, bgData, "h_met_scores_", Year,DNN_inner_edges, DNN_outer_edges,MET_inner_edges,MET_outer_edges,VR)
        plot_ABCD_ratios(
            outer_edge_results, 
            obs_0SVJ_data, pred_0SVJ_data, obs_0SVJ_sig, pred_0SVJ_sig, obs_0SVJ_bg, pred_0SVJ_bg,
            obs_1SVJ_data, pred_1SVJ_data, obs_1SVJ_sig, pred_1SVJ_sig, obs_1SVJ_bg, pred_1SVJ_bg,
        ...
        VR = 'VR2'
        output_dir = f'Nonclosure/WNAE/VRII/{Year}/ControlRegion'
        os.makedirs(output_dir, exist_ok=True)
        ...
        VR = 'VR3'
        output_dir = f'Nonclosure/WNAE/VRIII/{Year}/ControlRegion'
        os.makedirs(output_dir, exist_ok=True)
        DNN_outer_edges = np.linspace(0.3,0.85,30)
        DNN_inner_edges = np.linspace(0.1,0.6,30)
        MET_outer_edges = np.linspace(225,250,30)
